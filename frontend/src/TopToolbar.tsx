...
export default function TopToolbar(p: Props) {
  return <>
    <header className="titlebar"><div className="title-left"><strong>AdopyHzEditor</strong><span className="document-name">{p.audioName ?? "Blank workspace"}{p.dirty ? " •" : ""}</span>{p.menus}</div><span className={p.connected ? "status-pill on" : "status-pill"}>{p.connected ? "Python connected" : "Browser preview"}</span></header>
