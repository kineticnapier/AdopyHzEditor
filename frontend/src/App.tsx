...
import { BlankWorkspaceDialog, HarmonicDiagramDialog, LanguageDialog, QuickHzDialog, UpdateDialog } from "./ToolDialogs";
...
type ToolDialog = "blank" | "quickHz" | "harmonic" | "language" | "update" | null;
...
export default function App() {
  ...
  const [adoExportOpen, setAdoExportOpen] = useState(false); const [helpSection, setHelpSection] = useState<string | null>(null); const [toolDialog, setToolDialog] = useState<ToolDialog>(null);
  ...
  const menus = toolsApi ? <AppMenus hasSelection={selected.length > 0}
    onBlankWorkspace={() => setToolDialog("blank")}
    onLoadNotesOnly={() => stateAction(() => toolsApi.load_project_notes_only_dialog())}
    onMergeProject={() => stateAction(() => toolsApi.merge_project_notes_dialog())}
    onExportSelectedMidi={() => void toolsApi.export_selected_midi_dialog(selected).then((x) => setStatus(x.status))}
    onExportSelectedAdo={() => { setAdoExportOpen(true); setStatus("Enable ‘Selected notes only’ in ADOFAI Export"); }}
    onHarmonicDiagram={() => setToolDialog("harmonic")}
    onReanalyze={() => stateAction(() => toolsApi.reanalyze_audio())}
    onQuickHz={() => setToolDialog("quickHz")}
    onLanguage={() => setToolDialog("language")}
    onUpdates={() => setToolDialog("update")}
    onHelp={() => setHelpSection("quick_start")}
  /> : null;
  ...
  return <div className="app">
    ...
    {toolDialog === "blank" && toolsApi && <BlankWorkspaceDialog api={toolsApi} onClose={() => setToolDialog(null)} onStatus={setStatus} onState={(state) => { applyState(state); setSpectrum(null); }} />}
    {toolDialog === "quickHz" && toolsApi && <QuickHzDialog api={toolsApi} onClose={() => setToolDialog(null)} onStatus={setStatus} />}
    {toolDialog === "harmonic" && toolsApi && <HarmonicDiagramDialog api={toolsApi} selected={selected} onClose={() => setToolDialog(null)} onStatus={setStatus} onMutation={applyMutation} />}
    {toolDialog === "language" && toolsApi && <LanguageDialog api={toolsApi} onClose={() => setToolDialog(null)} onStatus={setStatus} />}
    {toolDialog === "update" && toolsApi && <UpdateDialog api={toolsApi} onClose={() => setToolDialog(null)} onStatus={setStatus} />}
  </div>;
}